...
- **可配置特征检测器** — 前端特征检测器可在 GFTT、FAST、SuperPoint 之间切换（通过 `feature_detector_type`），SuperPoint 基于 LibTorch 推理，支持多通道批量单次前向传播
- **可配置特征匹配器** — 前端特征匹配器可在 LK 光流、BRIEF+FLANN 之间切换（通过 `feature_matcher_type`），匹配器与检测器解耦
- **可配置图像滤波** — 8-bit 量化后可选地对 DoP/sin/cos 通道施加双边滤波、导向滤波或 NLM 滤波，抑制偏振传感器特有的噪声，提升暗光下特征检测稳定性
...
│       │   ├── feature_tracker_detector.h/cpp  # 特征检测器抽象（GFTT/FAST/SuperPoint）
│       │   ├── feature_tracker_matcher.h/cpp   # 特征匹配器抽象（LK/BRIEF+FLANN）
...
- ROS（Melodic / Noetic）
- OpenCV 3/4（BRIEF 匹配需要 opencv_contrib）
- Ceres Solver
- Eigen3
...
| 参数 | 说明 |
|------|------|
| `polar_filter_type` | 滤波器类型：0=无，1=双边滤波，2=导向滤波，3=NLM |
...
| `polar_nlm_h` | NLM 滤波强度参数（filter_type=3 时） |
| `polar_nlm_template` | NLM 模板窗口大小（filter_type=3 时） |
| `polar_nlm_search` | NLM 搜索窗口大小（filter_type=3 时） |
...
| 参数 | 说明 |
|------|------|
| `feature_detector_type` | 检测器类型：0=GFTT，1=FAST，2=SuperPoint |
...
| 参数 | 说明 |
|------|------|
| `feature_matcher_type` | 匹配器类型：0=LK 光流，1=BRIEF+FLANN |
...
前端通过 `FeatureMatcher` 抽象接口支持两种匹配器：

| 匹配器 | 类型值 | 说明 |
|--------|--------|------|
| LK_FLOW | 0 | Lucas-Kanade 光流 + 可选双向检查，原始 VINS-Fusion 默认 |
| BRIEF_FLANN | 1 | BRIEF/ORB 二值描述子 + FLANN LSH 索引（Hamming 距离 + 比值测试） |

BRIEF 需要 `opencv_contrib`；不可用时自动回退到 ORB 描述子。
...
| V2.3 (`master`) | 当前 | 在 V2.2 基础上集成 SuperPoint 检测器（LibTorch，PIMPL 模式，多通道批量推理） |
