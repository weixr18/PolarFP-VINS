...
struct PolarChannelResult {
    // --- 8-bit 可视化通道 ---
    cv::Mat S0_img;    ///< (h/2, w/2) S0 强度图，CV_8U
    cv::Mat S1_img;    ///< (h/2, w/2) 量化 S1 图 [0, 255], CV_8U
    cv::Mat S2_img;    ///< (h/2, w/2) 量化 S2 图 [0, 255], CV_8U
    cv::Mat dop_img;   ///< (h/2, w/2) 量化 DoP 图 [0, 255], CV_8U
    cv::Mat sin_img;   ///< (h/2, w/2) 量化 sin(AoP) 图 [0, 255], CV_8U
    cv::Mat cos_img;   ///< (h/2, w/2) 量化 cos(AoP) 图 [0, 255], CV_8U
    cv::Mat aop_vis;   ///< (h/2, w/2, 3) AoP 的 HSV 彩色可视化, CV_8UC3

    // --- double 精度原始值 ---
    cv::Mat S0_raw;    ///< (h/2, w/2) 原始 S0 值（double，范围 [0, 255]）
    cv::Mat S1_raw;    ///< (h/2, w/2) 原始 S1 值（double，范围 ≈ [-255, 255]）
    cv::Mat S2_raw;    ///< (h/2, w/2) 原始 S2 值（double，范围 ≈ [-255, 255]）
    cv::Mat aop;       ///< (h/2, w/2) 原始 AoP 值（double，单位：弧度）
    cv::Mat dop;       ///< (h/2, w/2) 原始 DoP 值（double，范围 [0, 1]）
};
 ...
 *   6. 对 DoP 做百分位归一化
 *   7. 量化输出为 8bit 图像
 *   8. 若 cfg.filter_type != FILTER_NONE，对 DoP/sin/cos 施加指定类型的滤波（仅支持导向滤波）
 *
